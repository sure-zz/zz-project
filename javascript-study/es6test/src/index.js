
//第一节：var全局声名 let 局部变量声名 const 常量声名
let a = 6;
console.log(a);
window.onload=function(){
	console.log(a);
}
{
	let a='技术胖的'
}
console.log(a);

for(let i=0;i<10;i++){
	console.log('循环体中')
}
console.log('循环本外')

//第二节：解构赋值
//一，数组的解构赋值
let [a1,b,c]=[1,2,3];
console.log(a1);
console.log(b);
console.log(c);
//陌认值undfined 取陌认值 null的区别 取null
let [foo=true]=[];
console.log(foo);
//陌认值undfined 取陌认值
let [a2,b2='技术胖']=['jspang',undefined];
console.log(a2+b2);
//null的区别 取null
let [a3,b3='技术胖']=['jspang',null];
console.log(a3+b3);

//二，对像解构赋值 key健词对象名 要对应上，数据是位置上，
let {foo_,bar}={foo_:'jspang',bar:"技术胖"};
console.log(foo_+bar);
//先赋值 再解构要用（）括住，不然会报错；
let foo1;
({foo1}={foo1:'jspang'})
//字符串解构 赋值
const [a4,b4,c4,d4,e4,f4]="jspang"
console.log(a4);
console.log(b4);
console.log(c4);
console.log(d4);
console.log(e4);
console.log(f4);

//第三节：扩展运算符
function jspang(...arg){
	console.log(arg[0]);
	console.log(arg[1]);
	console.log(arg[2]);
	console.log(arg[3]);
}
jspang(1,2,3);

let arr1=['www','jspang','com'];
// let arr2=arr1;
let arr2=[...arr1];
console.log(arr2);
arr2.push('shenghongyu');
console.log(arr1);

//和rest运算符 ... 三个点运算符
function jspang2(first,...arg){
	console.log(arg.length);
	//这种用 for of循环 更有利于内存资源的适放
	for(let val of arg){
		console.log(val);
	}
}
jspang2(0,1,2,3,4,5,6,7);

//第四节：字符串模板  字符串查找
let jspang1="技术胖"
let blog="大口工若大土日工若日工大口土日 技术胖"
document.write(blog.indexOf(jspang1)>0)
document.write(blog.includes(jspang1));
//查找开头
document.write(blog.startsWith(jspang1));
//结尾查找
document.write(blog.endsWith(jspang1));
//复制字符串
document.write('jspang|'.repeat(3));

//第五节课 ：数字操作
//二进制声明 binary
let binary=0b01010100;
console.log(binary);
//八进制声明 octal
let octal=0o6666
//判断一个数是不是数字
let anumber=11/4;
console.log(Number.isFinite('jspang'));
//NaN
console.log(Number.isNaN(NaN));
//Number.isInteger
let Inter=918.9
console.log(Number.isInteger(Inter));
//取值范围最大安全整数 isSafeInteger判断是否是一个整数
let jspang3=Math.pow(2,53)-1;
console.log(Number.isSafeInteger(jspang3))

//第六节 json 数组格式Array.form,Array.of
let json={
	'0':'jspang',
	'1':'技校在',
	length:2
}
let arr=Array.from(json);
console.log(arr)
//直接转成数组
let arr22=Array.of(1,2,4,5);
//find() 实例方法 先有arr实例，再调用方法
let arr33=[1,2,3,4,5,6,7,8,9];
console.log(arr33.find(function(value,index,arr){
	return value>5;
}));

//第七节 新增数据属性2
//fill 替换
let arr6=['jspang','技术胖','大胖bb叨扰']
arr6.fill('web',1,3);//从位置1替换到 3,结果是，技术胖，大胖bb叨扰，替换成web
console.log(arr6)
//数据循环 for of
for (let val of arr6.keys()){
	console.log(val);
	// 0,1,2
}
for (let val of arr6){
	console.log(val);
	// 'jspang','web','web'
}
for (let val of arr6.entries()){
	console.log(val);
	 // [0, "jspang"]
	 // [1, "web"]
	 // [2, "web"]
}
for (let [key,val] of arr6.entries()){
	console.log(key,val);
	 // 0 "jspang"
	 // 1 "web"
	 // 2 "web"
}
//数据循环 entries
let list=arr.entries();
console.log(list.next().value);


//第八节课，剪头涵数
function add(a,b=1){
	if(a==0){
		throw new error("a is error");
	}
	return a+b;
}
console.log(add.length);

console.log("********");

var add=(a,b=1)=>{
	return a+b;
}
console.log(a+b)

//第九节 涵数和数组补漏
//对象的涵数和解构json
let json11={
	a:'jspang',
	b:"技术腾的"
}
function fun(json11){
	console.log(a,b);
}
//in的用法 判断对象或数组中，是否存在某个值
let objn={
	a:'jspang',
	b:"技术腾的"
}
console.log('c' in objn);

//遍历数组的方法forEach some fifter map
let arrnber=['jspang','技术胖','前端视频'];
//数组转字符串
console.log(arr.toString())

console.log(arr.join('-'));

//第十节：es6中对象 
let name='jspang';
let skill='web';
let obj=(name,skill);
//key值的构建
let key='skill';
var obj22={
	[key]:'web'
}
console.log(obj22, 1);

;(function(){
	var obj = {a:1}
	var obj1 = {b:2,a:2}
	var obj2 = Object.assign(obj1,obj)
	console.log(obj2)
})();

//第十一节 Symbol 在对象中的作用 数据被保护 for in 不支持媒举
var objsymbol={name:'ad',skill:"web"}
let age=Symbol();
objsymbol[age]=17;
for (let item in objsymbol){
	console.log(objsymbol[item]);
}
console.log(objsymbol[age]);

//第十二节 set和weakSet数据结构 去重
let setarr=new Set(['jspang','技术胖'])
console.log(setarr);
//add 追加 重名不管用
setarr.add('kkk')


//第十三节 map 数据结构
let json17={
	name:'jspang',
	skill:'web'
};
console.log(json17.name)

var map= new Map();
map.set(json17,'iam');
console.log(map);
map.set('jspang',json17)
console.log(map);
//map 增删查
//取值get
console.log(map.get(json17));//获取
map.delete(json17);//删除
console.log(map);
console.log(map.size);//个数

console.log(map.has('jspang1'))//查找

//第十四节 用proxy进行预处理
//proxy 代理 es6 增强  对象和涵数（方法）vue 生命周期预处理
let objtt={
	add:function(val){
		return val+100;
	},
	name:'i am jspang'
}
console.log(objtt.add(100));
console.log(objtt.name);

let pro=new Proxy({
	add:function(val){
		return val+100;
	},
	name:'i am jspang'
},{
	//get set apply
	//得到之前预处理的一些东西
	get:function(target,key,property){
		console.log('come in get');
		return target[key]

	},
	set:function(target,key,value,receiver){
		console.log('setting ${key}=$(value)');
		return target[key]=value+'111';
	}
});
console.log(pro.name);
pro.name="技术胖";
console.log(pro.name);

//apply

let target=function(){
	return 'i anumber jspang';
}
let handler={
	  apply(target,ctx,args){
	  	 console.log("do apply");
	  	 return Reflect.apply(...arguments);
	  }
}
let pro11=new Proxy(target,handler);
console.log(pro11());


//第十七节 class类的使用
//class 的声名，和使用，里面都是方法 涵数  不用加‘,’逗号，这是坑
class coder{
	//val类涵数参数
	name(val){
		console.log(val);
		return val;
	}
	skill(val){
		//可以用this 用来调用 this不是指skill 还是coder
		console.log(this.name('技术腾')+'skill-'+val)
	}
	//用constructor 来设参数
	constructor(a,b){
		this.a=a;
		this.b=b;
	}
	add(){
		return this.a+this.b
	}
}
let jspang99=new coder(1,2);
jspang99.name('jspang1345');
//这样会有错undfind
jspang99.skill('web');

//类的继续
class htmler extends coder{

}
//实例化
let pang=new htmler;

//第十六节课 promise es5 回调地诳三步 
;(function(){
	let state=1;
	//resolve 同意 reject 拒绝
	function step1(resolve,reject){
		console.log('1.开始-洗菜做铁');
		if(state===1){
			resolve('洗菜做饭-完成');
		}else{
			reject('洗菜做饭-未完成')
		}
	}
	function step2(resolve,reject){
		console.log('2.开始-做下来吃饭');
		if(state===1){
			resolve('吃饭-完成');
		}else{
			reject('吃饭-未完成')
		}
	}
	function step3(resolve,reject){
		console.log('3.收拾-碗');
		if(state===1){
			resolve('洗碗-完成');
		}else{
			reject('洗碗-未完成')
		}
	}

	new Promise(step1).then(function(val){
		console.log(val);
		return new Promise(step2);
	}).then(function(val){
		console.log(val);
		return new Promise(step3);

	})
})();

//第十八节 es6模块化操作
//export输出。import 引入








